...
ssize_t input_buf(info_t *info, char **buf, size_t *len)
{
    ...
    if (!*len) /* Fill it if there is nothing left in the buffer, */
    {
...
ssize_t get_input(info_t *info)
{
    ...
    if (r == -1) /* EOF */
        return -1;
    if (len) /* Anycommands left in the chain buffer */
    {
        j = i; /* Initializes a new iterator to current buf position */
...
/**
 * read_buf - reads a buffer in program
 * @info: parameter struct
 * @buf: buffer
 * @i: size
 *
...
/**
 * sigintHandler - command that blocks ctrl-C
